...
# Petrolhead Masternode Setup Guide
***
## Required
1) **PTLH collateral value at current block** ([consult the collateral table](../../README.md#rewards-breakdown))
...
Remember to do `petrolhead-cli getblockcount` to check if VPS catching blocks till it synced with chain, if not follow this procedure:

* Go to your wallet-qt and check peers list (tools - peers list) and select one ip from the list. With that ip do the follow command at VPS `petrolhead-cli addnode "ip" onetry`

      Example:
		  petrolhead-cli addnode 45.32.144.158 onetry
    
* Check now if VPS already downloading blocks with the command `petrolhead-cli getblockcount`, and if yes give it time now to catch last block number 
...
```
petrolhead-cli getmasternodestatus
```

You need to get **"status" : 4** 

## Congratulations your Petrolhead node it's running
