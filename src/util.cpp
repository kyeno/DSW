...
const char * const PIVX_CONF_FILENAME = "petrolhead.conf";
const char * const PIVX_PID_FILENAME = "petrolhead.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";
const char * const PIVX_ACTIVE_MASTERNODE_CONF_FILENAME = "activemasternode.conf";


// Petrolhead only features
// Masternode
...
#else
    const char* pszModule = "petrolhead";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\petrolhead
// Windows >= Vista: C:\Users\Username\AppData\Roaming\petrolhead
// Mac: ~/Library/Application Support/petrolhead
// Unix: ~/.petrolhead
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Petrolhead";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty petrolhead.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override petrolhead.conf
