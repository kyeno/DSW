...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Petrolhead client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Petrolhead"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Petrolhead"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
